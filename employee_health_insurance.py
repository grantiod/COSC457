from tkinter import Entry, Button, Tk, END, mainloop, Label
import MySQLdb

class Employee_Health_Insurance:
    def __init__(self):
        self.root = Tk()
        self.root.title('Employee Health Insurance')

    # upload data to db
    def submit(self):
        db = MySQLdb.connect(
            host="localhost",
            user="root",
            passwd="Gman1212!"
        )

        employee_name = self.employee_name_e.get()
        self.employee_name_e.delete(0, END)
        employee_num = self.employee_num_e.get()
        self.employee_num_e.delete(0, END)
        depend_name = self.depend_name_e.get()
        self.depend_name_e.delete(0, END)
        insur_prov = self.insur_prov_e.get()
        self.insur_prov_e.delete(0, END)

        c = db.cursor()
        c.execute('USE psych_office')
        c.execute('SET FOREIGN_KEY_CHECKS = 0')
        c.execute("INSERT INTO EMPLOYEE_HEALTH_INSURANCE VALUES (\"" + employee_name + "\"," + employee_num + ",\"" + depend_name + "\",\"" + insur_prov + "\")")
        c.execute('SET FOREIGN_KEY_CHECKS = 1')
        db.commit()
        c.close()
        db.close()

    def run(self):
        self.employee_name_lbl = Label(self.root, text='Employee Name:')
        self.employee_name_lbl.grid(row=0, column=0, padx=5, pady=5)
        self.employee_name_e = Entry(self.root, width=10, borderwidth=1)
        self.employee_name_e.grid(row=0, column=1, padx=5, pady=5)

        self.employee_num_lbl = Label(self.root, text='Employee Num:')
        self.employee_num_lbl.grid(row=0, column=2, padx=5, pady=5)
        self.employee_num_e = Entry(self.root, width=10, borderwidth=1)
        self.employee_num_e.grid(row=0, column=3, padx=5, pady=5)

        self.depend_name_lbl = Label(self.root, text='Dependent Name:')
        self.depend_name_lbl.grid(row=0, column=4, padx=5, pady=5)
        self.depend_name_e = Entry(self.root, width=10, borderwidth=1)
        self.depend_name_e.grid(row=0, column=5, padx=5, pady=5)

        self.insur_prov_lbl = Label(self.root, text='Insurance Provider:')
        self.insur_prov_lbl.grid(row=0, column=6, padx=5, pady=5)
        self.insur_prov_e = Entry(self.root, width=10, borderwidth=1)
        self.insur_prov_e.grid(row=0, column=7, padx=5, pady=5)

        self.submit = Button(self.root, text='Submit', padx=5, pady=5, command=self.submit)
        self.submit.grid(row=1, column=0)

        mainloop()

if __name__ == '__main__':
    e = Employee_Health_Insurance()
    e.run()